...
pub fn main() !void {
    ...
    var arr = try std.ArrayList(u16).initCapacity(allocator, 64);
    for (set.data) |i| {
        if (i < 0) try arr.append(allocator, @intCast(-i));
    }
    const connects = try arr.toOwnedSlice(allocator);
    defer allocator.free(connects);
    std.mem.sort(u16, connects, {}, std.sort.desc(u16));

    var index: usize = 0;
    while (index < 3 and index < connects.len) : (index += 1) {
        res *= connects[index];
    }
